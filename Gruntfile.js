...
module.exports = function (grunt) {
		...
		replace: {
			...
			plugin_php: {
				...
				replacements: [{
					...
					from: /define\(\s*'WPLTPI_VERSION',\s*'(.*)'\s*\);/,
					to: "define( 'WPLTPI_VERSION', '<%= pkg.version %>' );"
				}]
			},
			plugin_js: {
				src: [ 'js/wp-link-to-playlist-item.js' ],
				overwrite: true,
				replacements: [{
					from: /@version (.*)/,
					to: "@version <%= pkg.version %>"
	...
	grunt.registerTask( 'version_number', [ 'replace:readme_txt', 'replace:plugin_php', 'replace:plugin_js' ] );
